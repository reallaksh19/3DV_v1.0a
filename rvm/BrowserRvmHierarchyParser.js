...
function classifyBranchSegment(segment, depth) {
  ...
  if (/^GASKET\b|^FLANGE\b|^VALVE\b|^ELBOW\b|^TEE\b|^RTORUS\b|^TORUS\b|^INSTRUMENT\b|^REDUCER\b|^CONE\b|^NOZZLE\b|^CAP\b|^STRAINER\b|^SUPPORT\b|^SLEEPER\b/.test(upper)) return 'COMPONENT_GROUP';
...
function semanticFromOwnerName(ownerName) {
  ...
  if (/\bSLEEPER\b|\bSKID\b/.test(upper)) return { type: 'SLEEPER', kind: 'SLEEPER', primitiveClass: 'SLEEPER' };
...
function renderPrimitiveFor(type, kind) {
  ...
  if (t === 'SLEEPER') return 'SUPPORT_BBOX_PLACEHOLDER';

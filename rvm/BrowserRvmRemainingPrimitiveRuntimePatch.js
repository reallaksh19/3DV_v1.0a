...
function publishDiagnostics(diagnostics = {}) {
  ...
  // Dispatched under both names: BrowserRvmTorusParityRuntimePatch listens for the
  // plural form while RvmPrimitiveFallbackBridge listens for the singular form.
  try { globalThis.dispatchEvent?.(new CustomEvent('rvm-remaining-primitive-diagnostics', { detail: payload })); } catch (_) {}
  try { globalThis.dispatchEvent?.(new CustomEvent('rvm-remaining-primitives-diagnostics', { detail: payload })); } catch (_) {}
  return payload;
}

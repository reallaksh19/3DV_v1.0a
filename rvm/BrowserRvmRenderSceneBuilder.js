...
export function buildInstructionObject(instruction = {}, options = {}, context = makeRenderContext(normalizeRenderOptions(options))) {
  ...
  } else if (!object && effectivePrimitive === 'DISH_DOME') {
    object = dishObject(center, axis, length, radius, material, context);
...
function dishObject(center, axis, length, radius, material, context = null) {
  const domeRadius = Math.max(radius * 1.1, 1);
  const geometry = cachedGeometry(context, `dish:${fixed(domeRadius)}:20:12`, () => new THREE.SphereGeometry(domeRadius, 20, 12, 0, Math.PI * 2, 0, Math.PI * 0.55));
  const mesh = new THREE.Mesh(geometry, material);
  mesh.position.copy(center);
  if (axis && axis.length() > 1e-6) mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), axis.clone().normalize());
  return mesh;
...
function effectivePrimitiveFor(rawPrimitive, instruction = {}) {
  ...
  if (primitive === 'SUPPORT_BBOX_PLACEHOLDER' || type === 'SUPPORT' || type === 'SLEEPER') return 'SUPPORT_STAND';
  ...
  if (primitive === 'DISH_BBOX_PLACEHOLDER') return 'DISH_DOME';

...
export const BROWSER_RVM_NATIVE_FACET_GROUP_PRIMARY_SCHEMA = 'browser-rvm-native-facetgroup-primary/v4-smart-civil-style-only';
...
export function buildNativeFacetGroupObject(instruction = {}, material = null, context = null) {
  ...
  if (!geometry) {
    // No usable polygon data even though decode succeeded (e.g. degenerate contours) -
    // this is the only case where we still fall back to a bbox/hidden proxy.
    if (smartCivilPolicy?.deferNativeTessellation) {
      ...
      if (proxy) {
        stampNativeFacetUserData(proxy, native, {
          ...
          renderQuality: 'smart-civil-code11-no-geometry-proxy',
        });
        proxy.userData = {
          ...(proxy.userData || {}),
        ...
        if (context?.stats) context.stats.smartCivilFacetDeferredCount = (context.stats.smartCivilFacetDeferredCount || 0) + 1;
        return proxy;
      }
    }
    return null;
  ...
  // Real decoded polygon data exists: always build the actual tessellated shape.
  // The civil policy only controls *display style* (solid / ghosted / wireframe /
  // default-hidden-but-toggleable) - it must never substitute a bbox proxy for
  // real geometry that we successfully decoded.
  const sizePolicy = nativeFacetDisplayPolicy(geometry, native);
  const policy = smartCivilPolicy?.deferNativeTessellation
    ? {
        ...sizePolicy,
        wireframeOnly: smartCivilPolicy.action === 'wireframe-proxy' ? true : sizePolicy.wireframeOnly,
        ghosted: smartCivilPolicy.action === 'hidden' ? true : sizePolicy.ghosted,
        policy: smartCivilPolicy.policy,
        reason: smartCivilPolicy.reason,
        renderQuality: smartCivilPolicy.action === 'wireframe-proxy' ? 'smart-civil-code11-wireframe-real' : 'smart-civil-code11-ghost-real',
      }
    : sizePolicy;
  ...
  if (smartCivilPolicy?.deferNativeTessellation) {
    result.userData = {
      ...
      ...smartCivilPolicyUserData(smartCivilPolicy),
      effectiveRenderPrimitive: 'RVM_NATIVE_FACET_GROUP',
      renderQuality: policy.renderQuality,
      // Real geometry now backs this object, so unlike a bbox proxy it is safe to
      // pick/select even when its default display is hidden/wireframed.
      pickable: true,
      selectable: true,
    };
    if (smartCivilPolicy.action === 'hidden') result.visible = false;
    if (context?.stats) {

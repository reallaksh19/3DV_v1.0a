...
import { createStageThreePreviewRenderer, renderStagePreview, disposeStageThreePreviewRenderer, fitStagePreviewToRenderPlan, fitStagePreviewToSelection, resetStagePreview, updateStagePreviewSelection, setStagePreviewSelectionCallback, setStagePreviewHoverCallback, setStagePreviewMeasureCallback } from '../stage/render/StageThreePreviewRenderer.js';
...
function activatePanel(handles, panel) { const showDiagnostics = panel === 'diagnostics'; handles.propertiesTab.classList.toggle('is-active', !showDiagnostics); handles.diagnosticsTab.classList.toggle('is-active', showDiagnostics); handles.propertiesPanel.hidden = showDiagnostics; handles.diagnosticsPanel.hidden = !showDiagnostics; }
...
function renderPreview(handles, state) {
  ...
  try { const preview = ensurePreviewRenderer(handles, state); preview.selectedRef = state.selectedRef; preview.forceFit = state.previewFitPending || preview.forceFit; renderStagePreview(preview, state.model, state.renderPlan); applyToolState(handles, state); state.previewFitPending = false; state.previewDiagnostics = []; }
...
function renderSidePanels(handles, state) { renderHierarchyTree(handles.hierarchyList, state.model, state.selectedRef); renderProperties(handles.propertiesPanel, state.model, state.selectedRef); renderDiagnostics(handles.diagnosticsPanel, state.model?.diagnostics, state.validationErrors, [...(state.renderPlan?.diagnostics || []), ...state.previewDiagnostics], state.workerDiagnostics); updateStatus(handles, state); activatePanel(handles, state.activePanel); }
...
function applyPreviewSelection(ref, handles, state) { if (!ref?.type || !ref?.id) return; state.selectedRef = ref; state.activePanel = 'properties'; state.statusText = `Selected ${ref.type} ${ref.id}`; if (state.previewRenderer) { state.previewRenderer.selectedRef = ref; updateStagePreviewSelection(state.previewRenderer, state.model); } renderSidePanels(handles, state); }
function toggleObjectVisibility(state, type, id, toggleButton) {
  const index = state.previewRenderer?.objectIndex;
  if (!index) return;
  const objects = type === 'primitive' ? [index.primitiveId.get(id)].filter(Boolean) : (index[`${type}Id`] || index.nodeId).get(id) || [];
  if (!objects.length) return;
  const nowHidden = objects[0].visible !== false;
  for (const object of objects) object.visible = !nowHidden;
  toggleButton.textContent = nowHidden ? 'Show' : 'Hide';
  toggleButton.setAttribute('aria-pressed', String(nowHidden));
  toggleButton.closest('.json-viewer-tree-item')?.classList.toggle('is-hidden-row', nowHidden);
  state.previewRenderer.renderer.render(state.previewRenderer.scene, state.previewRenderer.camera);
}
...
function resetForWorkerFile(state, fileName, statusText) { disposeWorkerClient(state); clearLoadedModel(state, fileName); state.workerDiagnostics = []; state.workerDiagnosticKeys = new Set(); state.activePanel = 'diagnostics'; state.statusText = statusText; }
...
function wireEvents(handles, state) {
  const onToolbarClick = (event) => {
    const action = event.target?.dataset?.action; if (!action) return;
    if (action === 'open-stage-json') handles.fileInput.click();
    if (action === 'open-rvm') handles.rvmFileInput.click();
    if (action === 'download-stage-model') downloadStageModel(handles, state);
    if (action === 'load-sample') loadSample(handles, state);
    if (action === 'fit' || action === 'reset' || action === 'fit-selection') runPreviewCommand(action, handles, state);
    if (action === 'show-diagnostics') state.activePanel = 'diagnostics';
    if (TOOL_ACTIONS[action]) setActiveTool(action, handles, state);
    if (action === 'tool-marquee') toggleMarquee(handles, state);
    if (action === 'tool-measure') toggleMeasure(handles, state);
    if (VIEW_ACTIONS[action]) applyViewPreset(action, handles, state);
    if (!['load-sample', 'fit', 'reset', 'fit-selection', 'open-rvm', 'download-stage-model', ...Object.keys(TOOL_ACTIONS), ...Object.keys(VIEW_ACTIONS), 'tool-marquee', 'tool-measure'].includes(action)) renderAll(handles, state);
  };
  const onFileChange = (event) => { const file = event.target.files?.[0]; if (file) loadStageFile(file, handles, state); };
  const onRvmFileChange = (event) => { const file = event.target.files?.[0]; if (file) loadRvmFile(file, handles, state); };
  const onQualityChange = () => { state.renderQuality = normalizeRenderQuality(handles.qualitySelect.value); state.statusText = `Render quality: ${state.renderQuality}`; updateRenderPlan(state); renderAll(handles, state); };
  const onTreeClick = (event) => {
    const toggle = event.target.closest?.('[data-visibility-toggle-type]');
    if (toggle) { toggleObjectVisibility(state, toggle.dataset.visibilityToggleType, toggle.dataset.visibilityToggleId, toggle); return; }
    const row = event.target.closest?.('[data-ref-type][data-ref-id]');
    if (!row) return;
    applyPreviewSelection({ type: row.dataset.refType, id: row.dataset.refId }, handles, state);
  ...
  const onRightTabClick = (event) => { const panel = event.target?.dataset?.panel; if (panel) { state.activePanel = panel; renderSidePanels(handles, state); } };
  const onDrop = (event) => { event.preventDefault(); const file = event.dataTransfer?.files?.[0]; if (!file) return; file.name.toLowerCase().endsWith('.rvm') ? loadRvmFile(file, handles, state) : loadStageFile(file, handles, state); };
  const onDragOver = (event) => event.preventDefault();
  handles.toolbar.addEventListener('click', onToolbarClick); handles.navRow.addEventListener('click', onToolbarClick); handles.fileInput.addEventListener('change', onFileChange); handles.rvmFileInput.addEventListener('change', onRvmFileChange); handles.qualitySelect.addEventListener('change', onQualityChange); handles.hierarchyList.addEventListener('click', onTreeClick); handles.treeFilterInput.addEventListener('input', onTreeFilter); handles.rightPanel.addEventListener('click', onRightTabClick); handles.shell.addEventListener('drop', onDrop); handles.shell.addEventListener('dragover', onDragOver);
  return () => { handles.toolbar.removeEventListener('click', onToolbarClick); handles.navRow.removeEventListener('click', onToolbarClick); handles.fileInput.removeEventListener('change', onFileChange); handles.rvmFileInput.removeEventListener('change', onRvmFileChange); handles.qualitySelect.removeEventListener('change', onQualityChange); handles.hierarchyList.removeEventListener('click', onTreeClick); handles.treeFilterInput.removeEventListener('input', onTreeFilter); handles.rightPanel.removeEventListener('click', onRightTabClick); handles.shell.removeEventListener('drop', onDrop); handles.shell.removeEventListener('dragover', onDragOver); };
}
...
export function renderViewer3DJson(root) {
  ...
  handles.qualitySelect.value = state.renderQuality; const cleanup = wireEvents(handles, state); renderAll(handles, state);

  ...
  target.innerHTML = `
    ...
    <div class="json-viewer-tool-card">
      ...
      <div class="json-viewer-panel-actions">
        ...
        <button type="button" class="json-viewer-mini-btn" data-action="tag-hide-all" ${tags.length ? '' : 'disabled'}>Hide All</button>
        <button type="button" class="json-viewer-mini-btn" data-action="tag-show-all" ${tags.length ? '' : 'disabled'}>Show All</button>
...
function renderTagRow(tag) {
  const hidden = Boolean(tag.hidden);
  return `
    <article class="json-viewer-tag-row severity-${esc(tag.severity || 'info')}${hidden ? ' is-hidden-tag' : ''}">
      <button type="button" data-tag-focus="${esc(tag.id)}"><b>${esc(tag.text || tag.id)}</b><span>${esc(hidden ? 'hidden' : tag.severity || 'info')} - ${esc(tag.ref?.type || 'point')}</span></button>
      <button type="button" data-tag-toggle="${esc(tag.id)}">${hidden ? 'Show' : 'Hide'}</button>
